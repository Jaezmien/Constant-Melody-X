...
-- Judgment tween commands.
	...
	function JudgmentTween(self)
		if CONSTMELODY.Profile.Get().Options_DefaultComboTween then
			self:zoom(.8) self:decelerate(.1) self:zoom(.75) self:sleep(.6) self:accelerate(.2) self:zoom(0)
		else
			self:zoom(1.2) self:bounceend(0.2) self:zoom(.75) self:sleep(0) self:accelerate(.2) self:zoom(0)
		end
	end
	function HoldTween(self)
		if CONSTMELODY.Profile.Get().Options_DefaultComboTween then
			...
			self:diffuse(1,1,1,1); self:zoom(.65); self:bounceend(0.2) self:zoom(.5); self:sleep(.5) self:decelerate(0.2); self:diffusealpha(0);
		end
	end
...
-- Judgment Font List
	...
	judgmentFontList = CONSTMELODY.Get.Judgments()
...
-- These will be the option rows available on the [nth] option screen. The 'NextScreen' row will be automatically added as long as there is more than 1 option screen.
	...
	playerOptions[1] = { 'SpeedType','SpeedNumber','Mini','Perspective','NoteSkin','Turn','JudgmentFont','Compare','Voice','Rate' }
	playerOptions[2] = { 'MetaMods1','MetaMods2','MetaMods3','Turn','Accel','Scroll','Effect','Appearance','Handicap','InsertTaps','InsertOther','Hide','Ghost','Compare','Measure','LifeBar' }
...
function GameplayUpdate(self)
	...
	for pn=1,2 do
		if Player(pn) then
			...
			if judge and Dead and Dead.State[pn] and not judge[pn].Dead then
...
ModsMaster.NoteSkin =		{ modlist = CONSTMELODY.Get.NoteSkins(), Select = 1 }
