...
function wasted:Setup()
    local aftsprite = self:GetChild("AftSprite")
    ...
    self:GetChildAt(2-1):cmd("stretchto,0,0,SCREEN_WIDTH,SCREEN_HEIGHT;diffuse,1,1,1,0;")
    self:GetChildAt(3-1):cmd("basezoomx,.8;basezoomy,.8;xy,SCREEN_CENTER_X,SCREEN_CENTER_Y;diffusealpha,0;fadeleft,.1;faderight,.1;")
end

function wasted:Prepare()
    self:GetChild("AftSprite"):SetTexture(stitch("lua.aftoverlay").texture)
...
function wasted:Dead()
    ...
    self:GetChildAt(2-1):cmd("diffusealpha,1;linear,.5;diffusealpha,0;")
    self:GetChildAt(3-1):cmd("sleep,2.25;diffusealpha,1;zoom,1.1;linear,.3;zoom,1;sleep,2;linear,1;diffusealpha,0;")
end
